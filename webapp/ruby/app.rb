...
module Isuconp
  class App < Sinatra::Base
    ...
    get '/@:account_name' do
      ...
      results = db.prepare("SELECT `p.id`, `p.user_id`, `p.body`, `p.created_at`, `p.mime`, `u.account_name` FROM `posts` AS p JOIN `users` AS u ON p.user_id = u.id WHERE u.del_flg = 0 AND `u.user_id` = ? ORDER BY `p.created_at` DESC LIMIT #{POSTS_PER_PAGE}").execute(
        user[:id]
      )
      posts = make_posts(results)
    ...
    get '/posts' do
      ...
      results = db.prepare("SELECT `p.id`, `p.user_id`, `p.body`, `p.created_at`, `p.mime`, `u.account_name` FROM `posts` AS p JOIN `users` AS u ON p.user_id = u.id WHERE u.del_flg = 0 AND `p.created_at` <= ? ORDER BY `p.created_at` DESC LIMIT #{POSTS_PER_PAGE}").execute(
    ...
    get '/posts/:id' do
      results = db.prepare("SELECT `p.id`, `p.user_id`, `p.body`, `p.created_at`, `p.mime`, `u.account_name` FROM `posts` AS p JOIN `users` AS u ON p.user_id = u.id WHERE u.del_flg = 0 AND `p.id` = ? ORDER BY `p.created_at` DESC LIMIT #{POSTS_PER_PAGE}").execute(
        params[:id]
      )
